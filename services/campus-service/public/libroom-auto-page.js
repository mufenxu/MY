...
(function initLibroomAutoPage() {
  ...
  section.insertAdjacentHTML("beforeend", `<section class="libroom-panel libroom-auto-panel"><div class="libroom-auto-head"><div><h3>自动预约任务</h3><p class="libroom-auto-hint">设置某一天的预约和开始尝试时间，系统会按候选空间和时段顺序自动尝试一次。</p></div><button id="autoCreate" type="button">新建任务</button></div><div id="autoTasks" class="libroom-auto-tasks"><p class="libroom-output">正在加载自动预约任务...</p></div><form id="autoForm" class="libroom-auto-form" hidden><div class="libroom-auto-form-head"><h4 id="autoFormTitle">新建自动预约任务</h4><button id="autoCancel" class="ghost" type="button">取消</button></div><div class="libroom-auto-grid"><label><span>任务名称</span><input id="autoName" maxlength="80" required></label><label class="libroom-switch"><input id="autoEnabled" type="checkbox" checked><span>启用任务</span></label><label><span>预约日期 <small id="autoDateWeekday" class="libroom-auto-weekday"></small></span><input id="autoDate" type="date" required></label><label><span>开始尝试预约时间</span><input id="autoTime" type="time" step="60" value="08:30" required></label></div><p class="libroom-auto-form-note">到达开始尝试时间后，系统只在预约日期执行一次；候选预约时段限定在 ${bookableStart} - ${bookableEnd}。</p><div class="libroom-auto-candidates-head"><h4>候选空间和时段（按顺序尝试）</h4><button id="autoAdd" class="ghost" type="button">添加候选</button></div><div id="autoCandidates" class="libroom-auto-candidates"></div><div class="libroom-auto-grid"><label><span>申请主题</span><input id="autoTitle" maxlength="80" required></label><label><span>联系电话</span><input id="autoMobile" inputmode="numeric" maxlength="11" pattern="\\d{11}" required></label><label class="libroom-wide"><span>申请内容</span><textarea id="autoContent" maxlength="500" rows="3" required></textarea></label><label class="libroom-switch"><input id="autoOpen" type="checkbox"><span>公开本次申请</span></label></div><div class="libroom-actions"><button id="autoSave" type="submit">保存任务</button></div><p id="autoStatus" class="libroom-status" role="status" aria-live="polite"></p></form></section>`);

  const n = Object.fromEntries(["tasks", "create", "form", "formTitle", "cancel", "name", "enabled", "date", "dateWeekday", "time", "candidates", "add", "title", "content", "mobile", "open", "save", "status"].map((key) => [key, section.querySelector(`#auto${key[0].toUpperCase()}${key.slice(1)}`)]));
  ...
  function fill(task = null) {
    n.name.value = task?.name || ""; n.enabled.checked = task ? Boolean(task.enabled) : true; n.date.value = task?.reservationDate || task?.startDate || date(); n.time.value = task?.executeTime || "08:30"; n.title.value = task?.title || ""; n.content.value = task?.content || ""; n.mobile.value = task?.mobile || ""; n.open.checked = Boolean(task?.open);
    n.candidates.replaceChildren(); (task?.candidates?.length ? task.candidates : [{ areaId: spaces[0]?.id, startTime: "09:00", endTime: "11:00" }]).forEach(candidateRow); updateDateWeekday();
  }
  ...
  function render() {
    ...
    n.tasks.innerHTML = tasks.map((task) => { const reservationDate = task.reservationDate || task.startDate || ""; const badge = task.enabled ? "待执行" : task.lastStatus === "succeeded" ? "已完成" : task.lastStatus ? "已结束" : "已停用"; return `<article class="libroom-auto-task" data-id="${esc(task.id)}"><div><div class="libroom-auto-task-title"><strong>${esc(task.name)}</strong><span class="libroom-auto-badge${task.enabled ? " is-enabled" : ""}">${badge}</span></div><p>预约日期：${esc(reservationDate || "未设置")}${reservationDate ? `（${esc(weekday(reservationDate))}）` : ""} · 开始尝试：${esc(task.executeTime)} · 候选 ${Array.isArray(task.candidates) ? task.candidates.length : 0} 个</p><p class="libroom-auto-result">最近执行结果：${esc(resultText(task))}</p></div><div class="libroom-auto-task-actions"><button class="ghost" data-action="edit">编辑</button><button class="ghost" data-action="toggle">${task.enabled ? "停用" : "启用"}</button><button class="ghost" data-action="delete">删除</button></div></article>`; }).join("");
