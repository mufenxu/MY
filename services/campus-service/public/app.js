...
function formatCardDealTime(item = {}, group = {}) {
  ...
  if (monthText.length >= 6 && dayText) {
    ...
    const month = monthText.slice(4, 6);
    const day = dayText.padStart(2, "0");
    return `${year}-${month}-${day}${timeText ? ` ${timeText}` : ""}`;
  }
...
function defaultFreeRoomSections() {
  const now = new Date();
