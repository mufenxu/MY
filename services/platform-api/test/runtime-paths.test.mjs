...
test('container layout resolves sibling services from the workspace root', () => {
  ...
  assert.equal(runtime.paths.portalWebLoginTicketStore, path.join(containerRoot, 'apps/admin-console/src/web-login-ticket-store.js'));
  assert.equal(runtime.paths.portalGoogleAccountStore, path.join(containerRoot, 'apps/admin-console/src/google-account-store.js'));
...
test('external mode does not resolve local service runtime paths', () => {
  ...
  assert.ok(paths.portalWebLoginTicketStore);
  assert.ok(paths.portalGoogleAccountStore);
});
