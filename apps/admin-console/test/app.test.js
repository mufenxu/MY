...
test('environment detail is protected while overview contains only its summary', async () => {
  const protectedConfig = { ...loadConfig({ NODE_ENV: 'development' }), authDisabled: false, metricsToken: 'm'.repeat(32) };
  const protectedApp = createApp({ config: protectedConfig });
  await withServer(protectedApp, async (origin) => {
    assert.equal((await fetch(`${origin}/api/environment`)).status, 401);
  });
  ...
  const environmentDiagnostics = {
    getReport: async (options) => {
      ...
      return {
        ...
        summary: { state: 'attention', total: 2, missing: 1, invalid: 0, restartRequired: 0, verificationFailed: 0 },
        groups: [],
        variables: [{ key: 'PLATFORM_SESSION_SECRET', displayValue: null }],
      };
    },
    getSummary: async (options) => {
      calls.push(['summary', options]);
      return { available: true, state: 'attention', total: 2, missing: 1, invalid: 0, restartRequired: 0, verificationFailed: 0 };
    },
  };
  const app = createApp({
    config: { ...loadConfig({ NODE_ENV: 'development' }), metricsToken: 'm'.repeat(32) },
    environmentDiagnostics,
  });
  await withServer(app, async (origin) => {
    const detail = await fetch(`${origin}/api/environment?refresh=1`);
    assert.equal(detail.status, 200);
    assert.equal((await detail.json()).variables[0].displayValue, null);

    const overview = await fetch(`${origin}/api/operations/overview`);
    assert.equal(overview.status, 200);
    const body = await overview.json();
    assert.equal(body.environment.missing, 1);
    assert.equal('variables' in body.environment, false);
  });
