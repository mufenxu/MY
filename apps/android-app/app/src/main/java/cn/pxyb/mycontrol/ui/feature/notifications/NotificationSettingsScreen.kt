...
fun NotificationSettingsScreen(
    ...
    AppSubPage(
        ...
        item(key = "quiet-title") { AppSectionHeader(title = "免打扰与专注", subtitle = "设置普通提醒的接收时段") }
        item(key = "quiet") {
            AppPanel {
        ...
        item(key = "severity-title") { AppSectionHeader(title = "告警接收级别", subtitle = "选择需要收到的告警范围") }
        item(key = "severity") {
        ...
        item(key = "subscriptions-title") { AppSectionHeader(title = "业务订阅", subtitle = "按功能选择需要关注的提醒") }
        ...
        item(key = "brief-title") { AppSectionHeader(title = "每日简报", subtitle = "汇总课程、待办与需要关注的事项") }
        item(key = "brief") {
            AppPanel {
        ...
        saveError?.let { message -> item(key = "save-error") { AppFeedbackBanner(message, error = true) } }
        item(key = "save") {
            AppButton(
                text = "保存设置",
                loading = saving,
