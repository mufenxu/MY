...
test('mongosh keeps option-like credentials attached to their option names', () => {
...
test('container MongoDB checks attach environment-backed credentials to their options', async () => {
  ...
  assert.doesNotMatch(initScript, /--password\s+"\$MONGO_INITDB_ROOT_PASSWORD"/);
  assert.match(compose, /--username="\$\$MONGO_INITDB_ROOT_USERNAME"/);
  assert.match(compose, /--password="\$\$MONGO_INITDB_ROOT_PASSWORD"/);
  assert.doesNotMatch(compose, /--password\s+"\$\$MONGO_INITDB_ROOT_PASSWORD"/);
});
