...
test('Sidecar configuration preserves unrelated values and generates a separate token', () => {
  const values = parseEnv(configureSidecar(base, 'configure', () => deployToken, '/srv/my-platform'));
...
test('release actions require complete GitHub configuration', () => {
  ...
  const values = parseEnv(configureSidecar(base, 'enable-actions', () => deployToken, '/srv/my-platform'));
...
test('disable returns the platform to read-only without deleting the deployment token', () => {
  const configured = configureSidecar(base, 'configure', () => deployToken, '/srv/my-platform');
