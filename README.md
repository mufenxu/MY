...
The workflow waits for the exact commit's CI result, smoke-tests immutable SHA candidates, and only then promotes deployment tags. The admin release center can trigger builds and persists verified build artifacts; production hosts continue to update images explicitly with Compose. See [`docs/release-center.md`](docs/release-center.md).
